...
import { AccessTokenInput } from "@/features/SetupView/FamilySharedGamesSelection/AccessTokenInput/AccessTokenInput.tsx";
...
const ClickableBox = (props: {
	...
	<div
		className={
			...
			(props.disable ? "pointer-events-none select-none " : "cursor-pointer ") +
			"bg-border p-3 py-5 rounded-[5px] hover:bg-gray-800 transition-colors duration-500 " +
			props.className
		}
...
const FamilySharedGamesSelection = (props: { visible: boolean }) => {
	...
	const handleAccessTokenSubmit = useCallback(
		(token: string) => {
	...
	return (
		<>
			<div className={"relative min-h-[300px] w-[250px]"}>
				<div
					className={
						"flex flex-col gap-4 mt-4 text-gray-300 text-2xl mb-5 absolute translate-x-[-50%] left-[50%] family-shared-selection-decision"
					}
				>
					<p
						className={
							"w-fit text-text-primary text-xl " +
					...
					>
						...
						<p className={"text-gray-600 text-sm w-[300px]"}>
							Only works if you have a family shared account and requires
				...
				<div
					className={
						"flex flex-col gap-4 mt-4 w-[250px] text-gray-300 text-2xl mb-5 absolute left-[50%] translate-x-[-50%] access-token-input items-center " +
						(!props.visible && "pointer-events-none select-none")
					}
				>
					<AccessTokenInput
			...
			<Tooltip>
				<TooltipTrigger asChild>
					<span
						...
						className={
							"flex gap-2 items-center text-2xl text-gray-400 mt-8 pb-1 " +
				...
				<TooltipContent side={"bottom"}>Revert to previous step</TooltipContent>
			</Tooltip>
		</>
	);
};
