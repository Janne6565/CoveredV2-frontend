...
const ExportModal = () => {
  ...
  const [exportedFilename, setExportedFilename] =
    useState<string>("export.zip");
  ...
  return (
    <>
      <div
        ref={backgroundRef}
        className={
          "fixed w-screen h-screen bg-black left-0 top-0 opacity-0" +
          (open && "cursor-pointer")
        }
        style={{
          overflowY: "scroll",
      ...
      <div
        ref={ref}
        className={
          "right-[15vw] bottom-[6vh] cursor-pointer max-h-[90vh] rounded bg-primary relative " +
          (!disabled && !open
            ? "pointer-events-auto hover:!bg-gray-900"
            : " !cursor-default overflow-y-auto overflow-x-clip")
        }
        style={{
          width: "170px",
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div
              ref={exportTextRef}
              className={
                "w-full h-full flex items-center justify-center absolute select-none " +
                (disabled ? "text-gray-500" : "!text-white")
              }
            >
              Export {Object.keys(favoriteCovers).length} cover
              {Object.keys(favoriteCovers).length > 1 ? "s" : ""}
            </div>
          </TooltipTrigger>
          <TooltipContent>
            {disabled
              ? "You need to select at least one favorite cover to export"
              : "Export your favorite covers to a ZIP file. Click to export."}
          </TooltipContent>
        </Tooltip>
        <div
          ref={modalContentRef}
          className={
            "w-[550px] h-[750px] flex flex-col items-center justify-evenly text-white absolute opacity-0 p-8"
          }
        >
            ...
            >
              <div className="text-sm space-y-2">
                ...
                <p className="text-xs text-gray-200 mt-2">
                  Open the downloaded ZIP file and extract all contents to the
                  folder above.
                </p>
                <button
                  onClick={(e) => {
          ...
          <div className="mt-4 w-full border-t border-gray-800 pt-4 flex flex-col items-center gap-3 pb-5">
            <p className="text-xs text-gray-400">Do you like this tool? Feel free to support me :)</p>
            <a
              href="https://www.buymeacoffee.com/jannekeipert"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="Buy me an ice cream on Buy Me a Coffee"
              className="inline-block"
            >
              <img
                src="https://img.buymeacoffee.com/button-api/?text=Buy%20me%20an%20ice%20cream&emoji=🍦&slug=jannekeipert&button_colour=5F7FFF&font_colour=ffffff&font_family=Inter&outline_colour=000000&coffee_colour=FFDD00"
