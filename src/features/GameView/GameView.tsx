...
const GameView = (props: { visible: boolean }) => {
  ...
  const isHydrationFinished = useRehydrationStatus();
  ...
  const currentPage = useMemo(
    ...
    [filteredAndSortedGames, currentPageNumber, itemsPerPage]
  );
  ...
  const { isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (userPreferences.includeFamily && !userPreferences.accessToken)
        throw Error("bad config");
      const fetchedGames = userPreferences.includeFamily
        ? await loadGamesFromFamily(
          userPreferences.steamId,
          userPreferences.accessToken!
        )
        : await loadGamesFromPlayer(userPreferences.steamId);
      setTotalGames(fetchedGames.length);
    ...
    enabled: props.visible && Object.keys(games).length === 0 && isHydrationFinished,
    retry: true,
    retryDelay: 200,
  ...
  const { isLoading: isCoversLoading } = useQuery({
    ...
    queryFn: async () => {
      const covers = await fetchCoversFromGames(
    ...
    <div className={"min-w-[60%] flex flex-col gap-6 mb-20"}>
      ...
      (filteredAndSortedGames.length === 0 &&
        ...
        <div className="flex flex-col items-center justify-center h-[25vh] gap-6">
          <ProgressAnimation
            progress={
              missingGames !== null && totalGames !== null
                ? ((totalGames - missingGames) / totalGames) * 100
                : 0
            }
          />
          <span className={"text-gray-400 text-center"}>
						Loading your Games... (
            {totalGames !== null && missingGames !== null
              ? totalGames - missingGames
              : "-"}
            /{totalGames}) <br />
						Approximate time remaining:{" "}
            {missingGames !== null
              ? normalizeTimeToMinutes(missingGames * 0.3)
              : "-"}{" "}
            minutes
					</span>
          <span className={"text-gray-600"}>
						You can reload or leave this page and come back later
					</span>
        </div>
      ) : (
        <>
              ...
              {props.visible &&
                currentPage?.map((game) => (
                  <GameCard
                    game={game}
                    key={game.game.uuid}
                    onClick={() => setOpenGame(game.game.uuid)}
                  />
                ))}
            </div>
          )}
      ...
      <GameCoverModal
        game={openGame ? games[openGame] : null}
        closeModalCallback={() => setOpenGame(null)}
      />
      <ExportModal />
    </div>
  );
