...
const coversSlice = createSlice({
	...
	reducers: {
		setCovers: (state, action: PayloadAction<Cover[]>) => {
			state.coversForGame = {};
			state.coversForGame = action.payload.reduce(
				(acc, cover) => {
		...
		addCover: (state, action: PayloadAction<Cover>) => {
			...
			state.coversForGame[action.payload.game_uuid][action.payload.uuid] =
				action.payload;
		},
	},
});

export const { setCovers, addCover } = coversSlice.actions;
