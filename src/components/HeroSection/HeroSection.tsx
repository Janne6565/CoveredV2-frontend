...
export const HeroSection = ({ title, subtitle, tagline }: HeroProps) => {
	...
	return (
		...
		>
			<h5 className="text-2xl text-secondary font-thin p-0">{subtitle}</h5>

			<h1 className="text-5xl text-text-primary p-0 pb-2">{title}</h1>

			<h5 className="text-2xl text-secondary font-thin p-0">{tagline}</h5>

			<div
				tabIndex={-1}
				className={
					"transition-[height] text-center flex duration-400 overflow-y-hidden gap-5 w-fit " +
					(userConfig.steamName ? "h-[24px]" : "h-[0px]")
				}
			>
				<Tooltip>
					<TooltipTrigger
						tabIndex={userConfig.steamName ? 0 : -1}
						className={
							"flex gap-2 items-center text-gray-400 justify-center w-fit"
						}
					>
						<h5
							className={
								"text-m font-thin p-0 transition-opacity duration-300 select-none " +
								(userConfig ? "opacity-100" : "opacity-0 pointer-events-none")
							}
						>
							{steamNameMirror ?? ""}
						</h5>
					</TooltipTrigger>
					<TooltipContent>Your Steam ID</TooltipContent>
				</Tooltip>
				<DropdownMenu>
